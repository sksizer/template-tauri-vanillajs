...
cd src-web && pnpm outdated 2>&1 || true; cd ..
...
If lint or test failures remain, read the failing code, fix it (the cause will typically be a bumped dep with a breaking change inside its declared range — pin that dep back in the relevant manifest if the fix is non-trivial), and re-run `just ci` until it passes.
...
- Body: two sections —
  - **JS packages bumped** (grouped by root / src-web, from the pnpm update output)
