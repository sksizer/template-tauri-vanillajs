...
Run `pnpm update --latest` at the root and in `src-web/`, but exclude every `@tauri-apps/*` package so they can be bumped atomically as one unit in step 4.
...
# In src-web/
cd src-web
cp package.json /tmp/src-web-pkg.json.bak
pnpm update --latest 2>&1
...
- If `LATEST_MAJOR > CURRENT_MAJOR`, upgrade the entire Tauri set to `LATEST_MAJOR` in ONE logical change:
  ...
  - `src-web/package.json`: set `@tauri-apps/api`, `@tauri-apps/plugin-opener`, and any other `@tauri-apps/*` to `^LATEST_MAJOR`.
  ...
  - Run `pnpm install` at root and in `src-web/`, and `cargo update` in `src-tauri/`.
...
- Body: four sections —
  - **JS packages upgraded** (grouped by root / src-web, with `old → new` version pairs)
